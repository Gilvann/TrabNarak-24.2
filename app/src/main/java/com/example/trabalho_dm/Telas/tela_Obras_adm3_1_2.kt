...
@Composable
fun tela_Obras_adm3_1_2(navController: NavController, colecao: String) {
    ...
    // Carregando dados do Firebase
        LaunchedEffect(Unit) {
            db.collection(colecao).get().addOnSuccessListener { snapshot ->
                obras.clear()
            ...
            if (obras.isEmpty()) {
                ...
                LazyColumn(
                    ...
                    items(obras) { obra ->
                        ObraItem(
                            obra = obra,
                            onEditClick = { navController.navigate("tela obra/${obra.id}/${colecao}") },
                            onDeleteClick = {
                                obraToDelete = obra
                ...
                Button(
                    onClick = { navController.navigate("adicionar/${colecao}") },
            ...
            if (showDialog.value && obraToDelete != null) {
                AlertDialog(
                    ...
                    confirmButton = {
                        TextButton(
                            onClick = {
                                db.collection(colecao).document(obraToDelete!!.id).delete()
