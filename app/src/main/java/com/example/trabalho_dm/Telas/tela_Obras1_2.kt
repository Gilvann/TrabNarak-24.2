...
fun ObraItem(obra: Obras, onClick: () -> Unit) {
    Row(
        ...
        TextButton(onClick = onClick) {
            Text(
                text = obra.titulo,
                fontSize = 20.sp,  // Tamanho da fonte ajustado
                color = Color.Black
            )
        }
    }
...
@Composable
fun tela_Obras1_2(navController: NavController, colecao: String){
                ...
                if (obras.isEmpty()) {
                    ...
                    LazyColumn(
                        ...
                        items(obras) { obra ->
                            ObraItem(
                                obra = obra,
                                onClick = { navController.navigate("Obra1/${obra.id}/${colecao}") })
        }} }
